...
class AdfsService
{
    ...
    public function logout(?string $returnTo = null, ?string $nameId = null, ?string $sessionIndex = null, ?string $nameIdFormat = null): void
    {
        ...
        // Log the redirect location header
        $redirectUrl = $this->samlAuth->getLastRequestURL();
        if ($redirectUrl) {
            Log::info("Redirect URL from OneLogin: " . $redirectUrl);
        } else {
            Log::warning("No redirect URL generated by OneLogin");
        }
    }
